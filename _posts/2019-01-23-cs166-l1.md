...
Since we divide the array into two parts each time, there will be $O(\log n)$ levels and $O(2^{\log n}) = O(n)$ nodes. Since we do one computation per node, the preprocessing time is $O(n)$ this way.
...
* Else if the node's range and the query range are disjoint, we return an 'infinite' value (eg. `float('inf')` for Python or `numeric_limits<int>::max()` for C++).
...
Pseudocode (as well as implementation) of this can be found in the [GeeksforGeeks post][geeks]. The correctness of this algorithm isn't immediately obvious. Proofs of correctness and time complexity ($O(\log n)$) will be added.

This has the same preprocessing and query time complexities as our first hybrid structure above. The two structures are quite different though, and it's likely that one will beat the other in practice due to real-world factors. In the following Jupyter notebook, running on the [xeus-cling][xeus-cling] C++ kernel and embedded here in static form, I implemented the segment tree and compared its performance with the Hybrid 1 structure described above. I test two different implementations of the segment tree, one using a node pointer model (linked representation) and one using an array model (sequential representation). Throughout the notebook, I discuss how different factors affect the time and space performance, and in the end I plot the performance results of the different implementations against each other. I use the performance results of the Hybrid 1 structure without including its implementation, which is part of [Problem Set 1][pset1].
